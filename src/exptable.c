...
void make_ExpTable()
{
	...
	for(vindex=0;vindex<VNMAX;vindex++){
		...
		// for ikr2 (facilitated)
		ikr2.Txr1ss[vindex] = 1.0/(1.0+exp(-(v+26.0+26.5)/7.0));
		//ikr2.Ttauxr1[vindex] = (450/(1.0+exp(-(v+45.0+26.5)/10.0)))*(6.0/(1.0+exp((v+30.0-26.5)/11.5)));
		ikr2.Ttauxr1[vindex] = (450/(1.0+exp((-45.0-v)/10.0)))*(6.0/(1.0+exp((v+30.0)/11.5)));
		...
		ical.Ttauf[vindex] = 0.5*(1102.5*exp(-(v+27.0)*(v+27.0)/225.0)+200.0/(1.0+exp((13.0-v)/10.0))+180.0/(1.0+exp((v+30.0)/10.0))+20.0);

		ical.Tf2ss[vindex] = 0.67/(1.0+exp((v+35.0)/7.0))+0.33;
		ical.Ttauf2[vindex] = 0.5*(600.0*exp(-(v+25.0)*(v+25.0)/170.0)+31.0/(1.0+exp((25.0-v)/10.0))+16.0/(1.0+exp((v+30.0)/10.0)));
