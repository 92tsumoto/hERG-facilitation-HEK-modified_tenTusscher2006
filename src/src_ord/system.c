...
void function(double x[],double f[],double t)
{
	...
	// Ikr (unfacilitated fraction)
	f[25] = var.Tf*(ikr.xrss - x[25])/ikr.tauxr_fast;
	f[26] = var.Tf*(ikr.xrss - x[26])/ikr.tauxr_slow;
	// Iks
	...
	f[41] = var.Tf*(ikr.xrss2 - x[41])/ikr.tauxr_fast2;
	f[42] = var.Tf*(ikr.xrss2 - x[42])/ikr.tauxr_slow2;
