...
void comp_ikr (double x[])
{
	...
	ikr2.xr1ss = ikr2.Txr1ss[iV]*d2 + ikr2.Txr1ss[iV+1]*d1;
	ikr2.tauxr1 = ikr2.Ttauxr1[iV]*d2 + ikr2.Ttauxr1[iV+1]*d1;

	// Drug effects
	if(var.drug_type == 0){
		ikr.without = var.normal_block*ikr.rategkr*ikr.Gkr*(x[0]-var.Ek)*x[6]*x[7];
		ikr.with = 0.0;
	} else if(var.drug_type == 1){
		ikr.without = var.normal_block*var.fraction_facil*ikr.rategkr*ikr.Gkr*(x[0]-var.Ek)*x[6]*x[7];
		ikr.with = var.normal_block*(1.0-var.fraction_facil)*ikr.rategkr*ikr.Gkr*(x[0]-var.Ek)*x[19]*x[7];
	}
